...
> 缘起:`planet-review-issues.md` 问题 ⑤ 提到 `orbitAngle` 的 JSDoc 注释把单位写成"角度",而代码里实际是"弧度"。这份文档讲清楚两者到底是什么、为什么有两种、项目里何时该用哪种。

![角度 vs 弧度](./degree-vs-radian.svg)
...
所以 `orbitAngle` 的 JSDoc 严格写成 `(单位: 弧度)` —— 这是 `planet-review-issues.md` 问题 ⑤ 要修的本质。

## 六、何时用 `THREE.MathUtils.degToRad / radToDeg`
