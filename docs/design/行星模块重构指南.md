...
原项目 [`src/planet/createPlanet.js`](https://github.com/rayallen20/universeBlog/blob/onlyWebpack/src/planet/createPlanet.js) 中的行星实现,采用的是「闭包工厂函数」模式 —— 一个 `createPlanet(config)` 函数内部创建并维护私有状态(`model` / `orbitAngle` / `pickableMeshes`),然后返回一个对象,该对象包含若干共享闭包的内层函数(`init` / `update` / `getPickableMeshes`)。
...
### 4.4 函数:`updatePlanet(planet, needRevolution)`
...
### 4.5 函数:`getPlanetPickableMeshes(planet)`
...
### 4.6 `planet.js` 完整 import 与 export
...
### 6.1 `initEngine` 中(在 `initSun` 之后,`initComposers` 之前)
...
### 6.2 `startAnimation` 中(在 `setSunAutoRotation` 之后,`renderBloomFrame` 之前)
...
### 7.1 `orbitAngle` 从「盒装对象 `{value}`」改成「数值」
...
### 7.4 `update` 接收 `needRevolution` 参数
