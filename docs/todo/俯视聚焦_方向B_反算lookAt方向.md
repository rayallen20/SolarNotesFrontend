...
本方向反过来做:
- 末态相机位置 `toCameraPosition` 沿用现有 `body + ttc × desireDistance` 计算
...
```
1. 已有: toCameraPosition = targetPosition + ttc × desireDistance
2. 计算 g = targetPosition - toCameraPosition          (从相机指向天体的向量,= -ttc × desireDistance)
3. 计算 D = |g| = desireDistance
4. 计算 α = asin(-g.y / D)                            (相机elevation,相机在天体之上时为正)
5. 计算 horizontalShiftRatio = min(PANEL_RATIO + MARGIN, MAX_SHIFT_RATIO)  (现有)
...
`initFocusAnimation` 内,把现有 §5.5 那段(从 `// 计算轨道控制器偏移向量` 开始到 `toControlsTarget.copy(...)` 结束)整段替换为:

```js
// ===== 方向 B: 反算 lookAtPoint,使 body 严格落在 NDC (X_target, 0) =====

// 从相机指向天体的向量(g 与 ttc 反向, |g| = desireDistance)
const gx = -targetToCameraDirection.x * desireDistance
const gy = -targetToCameraDirection.y * desireDistance
const gz = -targetToCameraDirection.z * desireDistance

// 相机相对天体的 elevation
const alpha = Math.asin(-gy / desireDistance)
...
// 可行域检查(α 太大时无解,见 §5 兜底)
const FEASIBILITY_MARGIN = 0.05  // 留 5% 余量避免数值边缘
if (sinAlpha > cosGamma - FEASIBILITY_MARGIN) {
    // §5 兜底:把 alpha 钳到可行域内,等价于先对 ttc 做一次"近平视" 钳制
    // (具体钳制细节按你选 A 还是 B 的最终方案补)
    // TODO: 临时直接 fallback 到原 doc 的钳制逻辑
    console.warn('focus: alpha 超出方向B可行域,临时fallback')
    // ...
}

// z_back 的 elevation
const betaZ = Math.asin(sinAlpha / cosGamma)
const sinBetaZ = Math.sin(betaZ)
const cosBetaZ = Math.cos(betaZ)

// g 的水平方向单位向量(在 XZ 平面)
const gHorizLen = Math.hypot(gx, gz)
const gHorizX = gHorizLen > 1e-6 ? gx / gHorizLen : 1
const gHorizZ = gHorizLen > 1e-6 ? gz / gHorizLen : 0

// z_back 水平方向相对 g_h 的方位角差
const cosPhiHg = -sinAlpha * cosBetaZ / (cosAlpha * sinBetaZ)
// sin 符号:先试 +,看效果不对再翻
const sinPhiHg = Math.sqrt(Math.max(0, 1 - cosPhiHg * cosPhiHg))

// 旋转 g_h 单位向量得到 z_back 水平方向(p_hat),绕世界 Y 轴
const pHatX = gHorizX * cosPhiHg - gHorizZ * sinPhiHg
const pHatZ = gHorizX * sinPhiHg + gHorizZ * cosPhiHg

// 合成 z_back(单位向量)
const zBackX = cosBetaZ * pHatX
const zBackY = sinBetaZ
const zBackZ = cosBetaZ * pHatZ

// lookAtPoint = camera - z_back × D
toControlsTarget.set(
    toCameraPosition.x - zBackX * desireDistance,
    toCameraPosition.y - zBackY * desireDistance,
    toCameraPosition.z - zBackZ * desireDistance,
)
```
...
console.log('focus debug:', {
    ...
    betaZ: THREE.MathUtils.radToDeg(betaZ).toFixed(1),
    gamma: THREE.MathUtils.radToDeg(gamma).toFixed(1),
    cosPhiHg: cosPhiHg.toFixed(3),
})
```
...
## 5. 兜底:α 超出可行域(elevation > ~50°)

`α > asin(cos(γ))` 时,数学上无解——意思是几何上**不存在一个 lookAtPoint 让 body 同时落在 NDC X = 0.8 且 NDC Y = 0**。这种情况通常是用户镜头拉到极俯视/极仰视。

兜底策略二选一:

### 5.1 兜底方案 ①:钳制 α 到可行域内

把 `α` 钳到 `α_max = asin(cos(γ)) - some_margin`(例如 45° 而不是 50.6°),然后**等效地修改 ttc 的 y 分量**:

```js
if (sinAlpha > cosGamma - FEASIBILITY_MARGIN) {
    const sinAlphaClamped = cosGamma - FEASIBILITY_MARGIN
    const cosAlphaClamped = Math.sqrt(1 - sinAlphaClamped * sinAlphaClamped)
    const scale = cosAlphaClamped / cosAlpha
    targetToCameraDirection.x *= scale
    targetToCameraDirection.z *= scale
    targetToCameraDirection.y = Math.sign(targetToCameraDirection.y) * sinAlphaClamped
    // 然后重新计算 toCameraPosition、g 等等
}
```

代价:用户从极俯视进入聚焦时,相机末态被压到 45° elevation,有视角跳变(类似方向 A 但只在极端情况触发)。
...
6. 调 `sinPhiHg` 符号(如果发现 body 在屏幕左侧而非右侧,翻一下加号变减号)
...
- [ ] **关键修复**:把镜头拉到 elevation 30°/45°(常见俯视角度)点击天体 → 末态 body 严格落在视口右侧(约 NDC X = 0.8)、垂直水平中线(NDC Y ≈ 0)
- [ ] **保留 elevation**:对比方向 A → **方向 B 末态相机仍维持用户原 elevation**,没有"突然下沉到近平视"的视角跳变(只有 elevation > ~50° 时兜底才出现钳制)
...
- [ ] **极俯视兜底**:elevation > 50° 触发聚焦 → 按 §5.1 钳到 45° / §5.2 降低 ratio,**不抛错、不出现 NaN、body 仍可见**
...
在 `α` 接近 50°(可行域边界)时,`β_z` 接近 90°,`sin(β_z) ≈ 1`,`cos(β_z) ≈ 0`,公式 `(2)` 的分母 `cos(α) × sin(β_z)` 趋于稳定,但分子 `sin(α) × cos(β_z)` 趋于零——`cos(φ_hg)` 数值上可能在 0 附近抖动。`FEASIBILITY_MARGIN = 0.05`(约 3°)是为了避免触碰这种数值边缘。**如果实测发现 elevation 50° 附近 body 跳动,把 margin 调大到 0.1**。
...
| 极端角度(elevation > 50°)处理 | 自然包含,统一钳到 15° | 需要单独兜底分支 |
